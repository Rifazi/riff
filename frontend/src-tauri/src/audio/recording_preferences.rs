...
pub fn get_default_recordings_folder() -> PathBuf {
    ...
    {
        // Windows: %USERPROFILE%\Music\riff-recordings
        if let Some(music_dir) = dirs::audio_dir() {
            music_dir.join("riff-recordings")
        } else {
            // Fallback to Documents if Music folder is not available
            dirs::document_dir()
                .unwrap_or_else(|| PathBuf::from("."))
                .join("riff-recordings")
        }
    }

    #[cfg(target_os = "macos")]
    {
        // macOS: ~/Movies/riff-recordings
        if let Some(movies_dir) = dirs::video_dir() {
            movies_dir.join("riff-recordings")
        } else {
    ...
    {
        // Linux/Others: ~/Documents/riff-recordings
        dirs::document_dir()
            .unwrap_or_else(|| PathBuf::from("."))
            .join("riff-recordings")
    }
