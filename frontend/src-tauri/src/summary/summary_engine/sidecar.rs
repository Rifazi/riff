...
impl SidecarManager {
    ...
    fn resolve_helper_binary() -> Result<PathBuf> {
        ...
        if let Ok(env_path) = crate::brand_migration::env_var("LLAMA_HELPER") {
            if !env_path.is_empty() {
                let path = PathBuf::from(env_path);
                if path.exists() {
                    log::info!("Using llama-helper from RIFF_LLAMA_HELPER: {}", path.display());
                    return Ok(path);
                }
            }
        }
        ...
        Err(anyhow!(
            "llama-helper binary not found. Build with 'cd llama-helper && cargo build --release' or set RIFF_LLAMA_HELPER env var."
        ))
    }
